...
# minimal helpers for the bootstrap phase (real ones come from _lib.sh below)
...
PLATFORM="$(resolve_platform "$PLATFORM")" || exit 1
...
pkg_install() { # pkg_install <cli-to-check> <mac-cask-or-formula> <arch-pkg> [--cask]
  ...
  if [[ "$PLATFORM" == mac ]]; then
    if [[ "$cask" == "--cask" ]]; then
      ...
      info "installing $mac_pkg"; brew install --cask "$mac_pkg"
    else
      brew list "$mac_pkg" >/dev/null 2>&1 && { ok "$mac_pkg already installed"; return; }
      info "installing $mac_pkg"; brew install "$mac_pkg"
    fi
  else
...
link() { # link <src> <dest> — backs up whatever is there first
  local src="$1" dest="$2" bak
  mkdir -p "$(dirname "$dest")"
  if is_our_link "$src" "$dest"; then ok "linked   $dest"; return; fi
  if [[ -L "$dest" ]]; then
    rm "$dest"
  elif [[ -e "$dest" ]]; then
    bak="$dest.bak"
    [[ -e "$bak" ]] && bak="$dest.bak-$(date +%Y%m%d-%H%M%S)"   # never clobber an old backup
    mv "$dest" "$bak"
...
bold "Configs"
while IFS=$'\t' read -r src dest; do link "$src" "$dest"; done \
  < <(link_map "$DOTFILES" "$PLATFORM")
